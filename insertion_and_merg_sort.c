...
void insertion_sort(int *a, int n) {
	...
	for(size_t i = 1; i < n; ++i) {
		int tmp = a[i];							// Create a temp var which will take initial array
		size_t j = i;							// size and count
		
		// Condition for values larger than 0 and if new array is less then temp
		while(j > 0 && tmp < a[j - 1]) {
			a[j] = a[j - 1];
			--j;								// decrement for next value
		count++;
		}
...
void merge_sort (int *a, int n) {
    ...
    if (n < 2)									// for generated arrays of three or more vals
        return;
   
    int m = n / 2;								// Divide and conquer method  
    merge_sort(a, m);							// Keeps dividing until reached single value to compare
    merge_sort(a + m, n - m);
    merge(a, n, m);								// calls for merge prototype
...
int main () {
	...
	int input;
	int i;
	printf("Enter length of array: ");			// Input for desired sizes
	scanf("%d", &input);
